...
При опечатке в имени pipeline также найдёт первый `.json`/`.txt` файл с `inst`
...
> **Каждый ключ работает обособленно.** Ключ не требует «соседей»: например,
> `privacy_policy_url` проставит политику даже без `app_info`; `marketing_url` /
> `support_url` / `whats_new_unique` применятся ко всем локалям даже без
> `distribution_page`; `push_notifications` включит capability и без `create_bundle`.
...
- Если точного имени нет, используется первый по пути `.json`/`.txt` файл с `inst`
  в имени; в лог добавляется предупреждение о нестандартном имени.
...
ввести другой bundle ID и продолжить тот же запуск. При повторном конфликте окно
появится снова; кнопка «Отмена» завершает весь запуск.
...
> **Имя API-ключа** чистится от пробелов и спецсимволов: `App: Name - Aaa` →
> `AppNameAaa` (+`{app_card_id}ApiKey`, до 30 символов).

> **`create_new_version`**: если редактируемая версия уже есть — при `yes` она
> используется как есть (без повторного инкремента), при явном номере —
> переименовывается в него. Если версии нет — создаётся новая.
...
Одни на всё приложение — применяются во **ВСЕ локали** (существующие + новые,
включая главную).
...
> - `privacy_policy_url` → отдельный шаг `privacy` проставит URL во все локали App Information;
> - `marketing_url` / `support_url` → если `distribution_page` не задан, отдельный шаг применит их во все локали версии.
>
> То же и с `whats_new_unique` (см. раздел Distribution) — работает без `distribution_page`.
...
> **Занятое имя или subtitle.** По умолчанию сервис добавляет к занятому полю
> невидимый Hangul Filler и повторяет попытку до лимита App Store в 30 символов.
> Верхнеуровневый ключ `app_info_symbol_filler: yes` переключает механизм на
> видимые символы: сначала пробуются ` +`, ` ++`, ... (либо такая же цепочка из
> точек), затем случайно перемешанные комбинации `+` и `.`. Ключ действует также
> на App Information, загруженный из `.docx` или `.xlsx`.
...
| `primary_subcategory_one` | str | 1-я подкатегория основной (для `GAMES`). |
| `primary_subcategory_two` | str | 2-я подкатегория основной. |
| `secondary_subcategory_one` | str | 1-я подкатегория вторичной. |
| `secondary_subcategory_two` | str | 2-я подкатегория вторичной. |
...
> **Подкатегории** актуальны прежде всего для `GAMES` (до 2 на каждую основную):
> `GAMES_ACTION`, `GAMES_ADVENTURE`, `GAMES_PUZZLE`, `GAMES_RACING`,
> `GAMES_STRATEGY`, `GAMES_ROLE_PLAYING`, `GAMES_SIMULATION`, `GAMES_SPORTS`,
> `GAMES_BOARD`, `GAMES_CARD`, `GAMES_CASINO`, `GAMES_FAMILY`, `GAMES_CASUAL`,
> `GAMES_TRIVIA`, `GAMES_WORD`, `GAMES_MUSIC`, `GAMES_EDUCATIONAL` и т.д.
> Подкатегория применяется только если задана её **основная** категория.
...
| `set_app_review_contact_as_account_holder` | bool | `yes` → весь контакт взять из данных аккаунта. |

> При `set_app_review_contact_as_account_holder: yes` все явные ключи
> `app_review_info_contact_*` **игнорируются**. Имя и фамилия берутся у
> ACCOUNT_HOLDER через `GET /v1/users`, телефон — из `account_phone_number`,
...
| `app_pre_order_release_date` | date | Дата релиза пре-ордера, `DD.MM.YYYY`. Apple допускает **2–180 дней** от сегодня; если выйти за диапазон — дата **зажимается** в границу (не падает). |
...
Верхнеуровневый ключ `set_up_billing_grace_period: yes` включает Billing Grace
Period сразу для всех подписок приложения: 16 дней, все renewals, Production и
Sandbox. Ключ указывается один раз рядом с `sub_products`, а не внутри блока.

```
set_up_billing_grace_period: yes
...
6. **categories** — категории + подкатегории (`primary_category`/`secondary_category` [+ `*_subcategory_*`])
...
15a. **billing_grace_period** — Billing Grace Period (`set_up_billing_grace_period`)
